...
def cvo_o3_get_file_v1(fn):
   ...
   return np.array(DT), np.array(DoY), np.array(ET), np.array(O3), np.array(flag) 
   
def cvo_o3_parse_data_v1(data):
   ix = len(data.DoY)
   for ii in range(0,len(data.DoY)):
      if data.DT[ii,1] != data.DT[0,1]:
         ix = ii
		 
   return data.DT[0:ix,:], data.DoY[0:ix], data.ET[0:ix], data.O3[0:ix], data.flag[0:ix]